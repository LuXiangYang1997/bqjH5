...
function inviteFriend(val){    
    ...
    if(val==1){
        burialPoint('website','shareActivityA11','10045');
        window.location.href="share.html"
    }else if(val==2){
        burialPoint('website','shareActivityB11','10047');
...
function receiveGift(val){
    if(val==1){
        burialPoint('website','receiveActivityA11','10046');
    }else if(val==2){
        burialPoint('website','receiveActivityB11','10048');
    }  
    let userInfo = Cookies.get("userInfo")?JSON.parse(Cookies.get("userInfo")):'';
    if(userInfo.length == 0){
        $("#bigModal").attr("style","display:block")
