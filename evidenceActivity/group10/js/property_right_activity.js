...
function inviteFriend(val){    
    ...
    if(val==1){
        burialPoint('website','shareActivityA10','10041');
        window.location.href="share.html"
    }else if(val==2){
        burialPoint('website','shareActivityB10','10043');
...
function receiveGift(val){
    if(val==1){
        burialPoint('website','receiveActivityA10','10042');
    }else if(val==2){
        burialPoint('website','receiveActivityB10','10044');
    }  
    let userInfo = Cookies.get("userInfo")?JSON.parse(Cookies.get("userInfo")):'';
    if(userInfo.length == 0){
        $("#bigModal").attr("style","display:block")
