...
function inviteFriend(val){    
    ...
    if(val==1){
        burialPoint('website','shareActivityA9','10037');
        window.location.href="share.html"
    }else if(val==2){
        burialPoint('website','shareActivityB9','10039');
...
function receiveGift(val){
    if(val==1){
        burialPoint('website','receiveActivityA9','10038');
    }else if(val==2){
        burialPoint('website','receiveActivityB9','10040');
    }  
    let userInfo = Cookies.get("userInfo")?JSON.parse(Cookies.get("userInfo")):'';
    if(userInfo.length == 0){
        $("#bigModal").attr("style","display:block")
