...
function inviteFriend(val){    
    ...
    if(val==1){
        burialPoint('website','shareActivityA8','10033');
        window.location.href="share.html"
    }else if(val==2){
        burialPoint('website','shareActivityB8','10035');
...
function receiveGift(val){
    if(val==1){
        burialPoint('website','receiveActivityA8','10034');
    }else if(val==2){
        burialPoint('website','receiveActivityB8','10036');
    }  
    let userInfo = Cookies.get("userInfo")?JSON.parse(Cookies.get("userInfo")):'';
    if(userInfo.length == 0){
        $("#bigModal").attr("style","display:block")
