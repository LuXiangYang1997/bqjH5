...
function inviteFriend(val){    
    ...
    if(val==1){
        burialPoint('website','shareActivityA1','10005');
        window.location.href="share.html"
    }else if(val==2){
        burialPoint('website','shareActivityB1','10007');
...
function receiveGift(val){
    if(val==1){
        burialPoint('website','receiveActivityA1','10006');
    }else if(val==2){
        burialPoint('website','receiveActivityB1','10008');
    }  
    let userInfo = Cookies.get("userInfo")?JSON.parse(Cookies.get("userInfo")):'';
    if(userInfo.length == 0){
        $("#bigModal").attr("style","display:block")
